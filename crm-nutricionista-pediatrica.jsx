...
async function criarEventoGoogle(token, payload) {
  ...
  if (!res.ok) throw new Error(`Falha ao criar evento no Google (${res.status})`);
  return res.json();
}
...
async function atualizarEventoGoogle(token, eventId, payload) {
  ...
  if (!res.ok) throw new Error(`Falha ao atualizar evento no Google (${res.status})`);
  return res.json();
}
...
async function excluirEventoGoogle(token, eventId) {
  ...
  if (!res.ok && res.status !== 410 && res.status !== 404) {
    throw new Error(`Falha ao excluir evento no Google (${res.status})`);
  }
}
...
/* ---------------------------------------------------------
   Follow-up (Kanban)
--------------------------------------------------------- */
function FollowupsBoard({ followups, patients, onMove, onAdd, onDelete }) {
  const [showModal, setShowModal] = useState(false);
  const patientName = (id) => patients.find((p) => p.id === id)?.nome || "—";
...
--------------------------------------------------------- */
function Integracoes({ googleClientId, setGoogleClientId, googleToken, gisReady, onConectar, onDesconectar, onSincronizarTudo, syncLog, onResetarBase }) {
